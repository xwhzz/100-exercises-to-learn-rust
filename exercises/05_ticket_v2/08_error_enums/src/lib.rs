    ...
    match ticket {
        ...
        Err(e) => {
            match e {
                ...
                TicketNewError::DesError(_) => Ticket::new(title, "Description not provided".to_string(), status).unwrap(),
            }
        }
    }
}
...
impl Ticket {
    pub fn new(
        ...
        if description.is_empty() {
            return Err(TicketNewError::DesError("Description cannot be empty".to_string()));
        }
        if description.len() > 500 {
            return Err(TicketNewError::DesError("Description cannot be longer than 500 bytes".to_string()));
        }
